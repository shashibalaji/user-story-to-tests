...
`
  ...
  if (categories && categories.length) {
    userPrompt += `\nIMPORTANT: Only generate test cases for these categories: ${categories.join(', ')}. Do NOT include any other categories.\n`
    userPrompt += `\nReturn only test cases for the selected categories. Do not include test cases for categories that are not selected.`
  } else {
